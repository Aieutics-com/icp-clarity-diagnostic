...
export async function POST(request: Request) {
  try {
    ...
    if (!body.name || !body.email || !body.shareUrl) {
    ...
    // Subject line with optional company name
    const subjectSuffix = body.companyName ? ` — ${body.companyName}` : "";
    ...
    <div style="background: #FFF3EC; border: 1px solid #FF5F1F; border-radius: 8px; padding: 20px; margin-bottom: 28px;">
      ...
      <a href="mailto:hello@aieutics.com?subject=ICP%20Clarity%20Diagnostic%20%E2%80%94%20Follow-up" style="display: inline-block; padding: 10px 24px; background: #FF5F1F; color: #ffffff; font-family: Georgia, 'Times New Roman', serif; font-size: 13px; font-weight: 700; text-decoration: none; border-radius: 8px;">
        Get in touch
      </a>
    </div>`
        : "";
  ...
  <div style="max-width: 560px; margin: 0 auto; padding: 40px 24px;">
    ...
    <div style="margin-bottom: 24px;">
      ...
      <p style="font-size: 14px; color: #6b6b6b; margin: 0; line-height: 1.6;">
        Thank you for taking the ICP Clarity Diagnostic!
      </p>
    </div>
    ...
    <div style="margin-bottom: 28px;">
      ...
      <p style="font-size: 14px; color: #6b6b6b; margin: 12px 0 0 0; line-height: 1.6;">
        This is a clarity profile, not a grade. The pattern of your scores matters more than the total.
      </p>
    </div>
    ...
    const text = `ICP Clarity Diagnostic — Results${body.companyName ? ` for ${body.companyName}` : ""}
...
https://aieutics.com`;
    ...
    const { error } = await getResend().emails.send({
      from: "ICP Clarity Diagnostic <hello@aieutics.com>",
      to: [body.email],
      bcc: ["hello@aieutics.com"],
      replyTo: "hello@aieutics.com",
      subject: `ICP Clarity Diagnostic${subjectSuffix}`,
      html,
      text,
    });
